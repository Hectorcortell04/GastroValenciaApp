...
// UseCase encargado de obtener los datos de los eventos. Se comunica con el repositorio
class EventsUseCase(private val repository: EventRepository = EventRepository()) {
    ...
    suspend fun getEventById(eventId: String, userId: String): Event =
        repository.getEventById(eventId, userId)

    suspend fun toggleLike(eventId: String, userId: String): Unit =
        repository.likeEvent(eventId, userId)
